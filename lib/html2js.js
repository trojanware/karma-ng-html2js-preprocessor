...
var createHtml2JsPreprocessor = function (logger, basePath, config) {
  ...
  var stripContentPrefix = new RegExp('^' + (config.stripContentPrefix || ''))
  var stripContentSuffix = new RegExp((config.stripContentPrefix || '') + '$')
  ...
  return function (content, file, done) {
    ...
    var strippedContent = escapedContent.replace(stripContentPrefix, '');
    strippedContent = escapedContent.replace(stripContentSuffix, '');
    if (moduleName) {
      tpl = util.format(SINGLE_MODULE_TPL, moduleName, moduleName, htmlPath, strippedContent))
    } else {
      tpl = util.format(TEMPLATE, htmlPath, htmlPath, strippedContent)
    }
